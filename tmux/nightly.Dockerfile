...
RUN mkdir -p "${HOME}/musl" && \
    cd "${HOME}/musl" && \
    curl -sSLfk -o "musl-${MUSL_VERSION}.tar.gz" "https://github.com/zydou/musl/archive/${MUSL_VERSION}.tar.gz" && \
...
RUN cp "${HOME}/bin/tmux" "${HOME}/bin/tmux-upx" && \
    curl -sSLf -o "upx-${UPX_VERSION}.tar.xz" "https://github.com/upx/upx/releases/download/${UPX_VERSION}/upx-${UPX_VERSION#v}-amd64_linux.tar.xz" && \
